...
IX_MODULE_CLBK::create_resource(OfObject& object, const int& resource_id, void *data)
{
    if (resource_id == ModuleGeometry::RESOURCE_ID_GEOMETRY)
    {
        ...
        if (HAPI_GetAttributeInfo(session, asset_id, object_id, geo_id, part_id, "P", HAPI_ATTROWNER_POINT, &attr_info_p) != HAPI_RESULT_SUCCESS) return 0;
        if ((attr_info_p.count == 0) || (attr_info_p.tupleSize != 3)) return 0;
        ...
        if (HAPI_GetAttributeInfo(session, asset_id, object_id, geo_id, part_id, "Pw", HAPI_ATTROWNER_POINT, &attr_info_pw) != HAPI_RESULT_SUCCESS) return curve_mesh;
        else if ((attr_info_pw.count == 0) || (attr_info_pw.tupleSize != 1) || (attr_info_pw.count != attr_info_p.count)) return curve_mesh;
